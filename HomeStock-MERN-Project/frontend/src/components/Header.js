...
function Header() {
  ...
  const handleVoiceCommand = (command) => {
    ...
    if (command.startsWith("add item")) {
      const itemName = words.slice(2, words.length - 2).join(" ");
      const quantity = parseInt(words[words.length - 1]) || 1;
      addItemToInventory(itemName, quantity);
    }

    if (command.startsWith("delete item")) {
      const itemName = words.slice(2).join(" ");
      deleteItemFromInventory(itemName);
    }

    if (command.startsWith("edit item")) {
      const itemName = words.slice(2, words.length - 2).join(" ");
      const newQuantity = parseInt(words[words.length - 1]) || null;
      editItemInInventory(itemName, newQuantity);
    }
  };
  ...
  const editItemInInventory = async (itemName, newQuantity) => {
    if (!itemName) {
      speak("Item name is required.");
      return;
    }
